(function () {
  ...
  const STATUS_LIST = ["未提交", "待初审", "初审中", "初审未通过", "专家评审", "审核未通过", "审核通过", "已归档"];
  ...
  const PROJECTS = [
    ...
    { id: "p9", status: "未提交", name: "冷链物流仓配与运输优化", belongUnit: "某冷链物流企业", projectStage: "商业化", trlLevel: "TRL 8", selfLevel: "A", resultTypes: ["软件系统", "技术方案"], deployForm: "云", valueProposition: "面向冷链温控与时效场景，基于仓配网络与路由优化、温控监控，解决成本与质量时效兼顾。", techHighlights: ["仓配网络与路由优化", "温控监控与预警", "应急调度与 SOP"], landingCaseCount: 7, typicalClient: "某生鲜电商", keyMetric: "运输成本单位降 12%", applyTime: "2024-05-08", industry: "食品" },
  ...
  var BUILDING_ICON = '<svg class="dashboard-project-card__icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 21h18"/><path d="M5 21V7l7-4 7 4v14"/><path d="M9 21v-6h6v6"/></svg>';
  var TREND_ICON = '<svg class="dashboard-project-card__metric-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>';
  var CALENDAR_ICON = '<svg class="dashboard-project-card__date-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>';

  function renderProjects(list, container, countEl, stateEl) {
    if (list.length === 0) {
      stateEl.innerHTML = '<p class="dashboard-projects-empty">暂无符合条件的项目，可尝试调整搜索或筛选条件。</p>';
      stateEl.classList.add("dashboard-projects-state--visible");
      container.innerHTML = "";
    } else {
      stateEl.innerHTML = "";
      stateEl.classList.remove("dashboard-projects-state--visible");
      container.innerHTML = list
        .map(function (p) {
          var statusSlug = (p.status || "").replace(/\s+/g, "");
          var statusClass = "dashboard-project-card__status dashboard-project-card__status--" + escapeHtml(statusSlug);
          ...
          var deployRow = deployLabel ? '<span class="dashboard-project-card__deploy" title="部署形态">' + escapeHtml(deployLabel) + '</span>' : "";

          var accentClass = "";
          if (p.status === "专家评审") accentClass = " dashboard-project-card--accent-orange";
          else if (p.status === "待初审") accentClass = " dashboard-project-card--accent-blue";

          var badges = [];
          if (p.trlLevel) badges.push('<span class="dashboard-project-card__badge">' + escapeHtml(p.trlLevel) + '</span>');
          if (p.selfLevel) badges.push('<span class="dashboard-project-card__badge">' + escapeHtml(p.selfLevel) + '</span>');
          if (p.projectStage) badges.push('<span class="dashboard-project-card__badge">' + escapeHtml(p.projectStage) + '</span>');
          var badgeRow = badges.length ? '<div class="dashboard-project-card__badges">' + badges.join("") + '</div>' : "";

          var resultTypes = (p.resultTypes || []).slice(0, 4);
          var solutionRow = resultTypes.map(function (t) {
            return '<span class="dashboard-project-card__solution"><span class="dashboard-project-card__solution-dot"></span>' + escapeHtml(t) + '</span>';
          }).join("");

          var valueProp = p.valueProposition ? '<p class="dashboard-project-card__value">' + escapeHtml(p.valueProposition) + '</p>' : "";
          ...
          if (hasCase || hasClient) {
            statsTopHtml =
              '<div class="dashboard-project-card__stats-top">' +
              (hasCase ? '<div class="dashboard-project-card__stat"><span class="dashboard-project-card__stat-label">落地案例</span><span class="dashboard-project-card__stat-value"><span class="dashboard-project-card__stat-num">' + escapeHtml(caseNum) + '</span><span class="dashboard-project-card__stat-unit">个</span></span></div>' : '') +
              (hasClient ? '<div class="dashboard-project-card__stat"><span class="dashboard-project-card__stat-label">典型客户</span><span class="dashboard-project-card__stat-value">' + escapeHtml(p.typicalClient) + '</span></div>' : '') +
              '</div>';
          }
          var metricHtml = p.keyMetric ? '<div class="dashboard-project-card__metric">' + TREND_ICON + '<span>' + escapeHtml(p.keyMetric) + '</span></div>' : "";
          var statsHtml = (statsTopHtml || metricHtml) ? '<div class="dashboard-project-card__stats">' + statsTopHtml + (statsTopHtml && metricHtml ? '<div class="dashboard-project-card__stats-sep"></div>' : '') + (metricHtml ? '<div class="dashboard-project-card__stats-bottom">' + metricHtml + '</div>' : '') + '</div>' : "";

          var dateHtml = p.applyTime ? '<div class="dashboard-project-card__date">' + CALENDAR_ICON + '<span>' + escapeHtml(p.applyTime) + '</span></div>' : "";

          return (
            '<article class="dashboard-project-card' + accentClass + '" role="listitem" data-id="' + escapeHtml(p.id) + '">' +
              '<div class="dashboard-project-card__top">' +
                (statusRow ? '<div class="dashboard-project-card__status-wrap">' + statusRow + '</div>' : '') +
                deployRow +
              '</div>' +
              '<h2 class="dashboard-project-card__title">' + escapeHtml(p.name || "") + '</h2>' +
              '<p class="dashboard-project-card__unit">' + BUILDING_ICON + '<span>' + escapeHtml(p.belongUnit || "") + '</span></p>' +
              badgeRow +
              valueProp +
              (solutionRow ? '<div class="dashboard-project-card__solutions">' + solutionRow + '</div>' : '') +
              statsHtml +
              '<div class="dashboard-project-card__footer">' +
                dateHtml +
                '<a href="project-detail-2.html?id=' + encodeURIComponent(p.id) + '" class="dashboard-project-card__link">查看详情 →</a>' +
              '</div>' +
            '</article>'
          );
        })
